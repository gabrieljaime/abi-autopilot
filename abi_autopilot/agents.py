...
def _load_prompt(base_dir: Path, name: str) -> str:
...
def run_codex(base_dir: Path, cfg: dict, worktree: Path, issue, base_sha: str, run_dir: Path,
    ...
    codex=cfg["codex"]
    template=_load_prompt(base_dir,"fixer.md" if mode=="fix" else "implementer.md")
...
def _claude_outer(stdout: str) -> dict:
    try:
        ...
        raise CommandError(f"Claude devolvió JSON externo inválido: {exc}", output=stdout or "")
    if not isinstance(outer, dict):
        raise CommandError("Claude devolvió una respuesta externa no-objeto", output=stdout or "")
    return outer
...
def _extract_review_from_outer(outer: dict) -> dict:
    ...
    if isinstance(structured, dict):
        ...
        if not isinstance(result, str) or not result.strip():
            raise CommandError("Reviewer no devolvió structured_output ni campo result utilizable", output=json.dumps(outer, ensure_ascii=False))
        review = _extract_json_object(result)
    if review.get("verdict") not in ("PASS", "FAIL"):
        raise CommandError("Reviewer no devolvió verdict PASS/FAIL", output=json.dumps(review, ensure_ascii=False))
    return review
...
def run_claude_review(base_dir: Path, cfg: dict, worktree: Path, issue, base_sha: str,
    ...
    claude = cfg["claude"]
    template = _load_prompt(base_dir, "reviewer.md")
...
"""
    ...
    while True:
        ...
        if _claude_hit_max_turns(outer):
            ...
            if not session_id:
                raise CommandError("Claude alcanzó max_turns pero no devolvió session_id para reanudar", p.returncode, (p.stdout or "") + "\n" + (p.stderr or ""))
            if attempts >= max_resumes:
                raise CommandError(
                    f"Claude reviewer agotó max_turns incluso después de {max_resumes} reanudaciones",
                    p.returncode,
                    (p.stdout or "") + "\n" + (p.stderr or ""),
                )
            attempts += 1
...
"""
            print(f"\nCLAUDE REVIEW · max_turns alcanzado; reanudando misma sesión ({attempts}/{max_resumes}) con {resume_turns} turnos. No cuenta como review/fix loop.")
            continue

        # Quota / transient failures are classified by the caller from this raw output.
        raise CommandError("Claude reviewer falló", p.returncode, (p.stdout or "") + "\n" + (p.stderr or ""))
