...
def run_capture(args: list[str], cwd: str | Path | None = None, input_text: str | None = None,
    ...
    try:
        ...
        if isinstance(e.stderr, bytes):
            ...
            out += "\n" + str(e.stderr)
        raise CommandError(f"Timed out after {timeout}s: {' '.join(map(str, args))}", 124, out) from e
    except OSError as e:
        ...
        if check:
            raise CommandError(f"Could not run: {' '.join(map(str, args))}: {detail}", 127, detail) from e
        return subprocess.CompletedProcess(args=list(args), returncode=127, stdout="", stderr=detail)
    if check and p.returncode != 0:
        out = (p.stdout or "") + ("\n" + p.stderr if p.stderr else "")
        raise CommandError(f"Command failed ({p.returncode}): {' '.join(map(str, args))}", p.returncode, out)
    return p
