...
def is_code_change(changed: list[str], cfg: dict | None = None) -> bool:
    """True si el diff toca algo que amerita validar.

    Con `integration.code_paths` (prefijos, ej. `["src/", "backend/"]`) sólo
    cuentan esas rutas. Sin configurar, cuenta cualquier archivo que no sea
    documentación: ante la duda se valida.
    """
    code_paths = [p for p in (cfg or {}).get("integration", {}).get("code_paths") or [] if p]
    if code_paths:
        return any(x.startswith(tuple(code_paths)) for x in changed)
    return any(not _is_doc(x) for x in changed)


def _condition_matches(condition: str, changed: list[str]) -> bool:
    if condition in (None, "", "always"):
        return True
    # Genérico: "changed:<prefijo>" corre el check sólo si el diff toca esa ruta.
    if condition.startswith("changed:"):
        prefix = condition[len("changed:"):].strip()
        return any(x.startswith(prefix) for x in changed)
    ...
    if condition == "docs_changed":
        return any(_is_doc(x) for x in changed)
    return True
...
def _with_worker_limits(command: str, name: str, cfg: dict) -> str:
    ...
    Pytest parallelism is opt-in because it requires pytest-xdist in the
    target repository. Keeping pytest=1 is always safe. Vitest can be bounded
    directly. Playwright defaults to 1 because E2E suites often share
...
def summarize(results: list[CheckResult]) -> str:
    if not results:
        return "No checks applied to the changed files."
    lines = []
    for r in results:
        if r.ignored:
