...
def remote_branch_sha(repo_path: str, branch: str) -> str:
    ...
    if not line:
        raise CommandError(f"origin/{branch} does not exist")
    return line.split()[0]
...
def create_or_reuse_worktree(repo_path: str, root: str, issue_number: int, title: str, base_branch: str):
    ...
    if wt.exists():
        ...
        if p.returncode == 0:
            ...
            if current != branch:
                raise CommandError(f"Worktree {wt} exists on branch {current}, expected {branch}")
            if not meta_file.exists():
                raise CommandError(f"Existing worktree without Autopilot metadata: {meta_file}. Check it manually.")
            meta=json.loads(meta_file.read_text(encoding="utf-8"))
            original_base=meta.get("base_sha")
            if not original_base:
                raise CommandError("Worktree metadata has no base_sha")
            return wt,branch,original_base,False
        raise CommandError(f"{wt} exists but does not look like a valid Git worktree")
    ...
    if exists:
        raise CommandError(f"Local branch {branch} already exists. Run cleanup or check it manually.")
    ...
    for f in changed:
        ...
        for p in protected:
            ...
            if pp.endswith("/"):
                ...
                bad.append(norm)
    if bad:
        raise CommandError("Protected paths were modified: " + ", ".join(sorted(set(bad))))
...
def fast_forward_remote_base(worktree: str | Path, commit_sha: str, base_branch: str, expected_base_sha: str):
    ...
    if current != expected_base_sha:
        raise CommandError(f"Base changed: expected {expected_base_sha[:12]}, origin/{base_branch} is at {current[:12]}")
    ...
    if parent != expected_base_sha:
        raise CommandError("The issue commit's direct parent is not the base SHA; an automatic fast-forward is not safe.")
    ...
    if verify != commit_sha:
        raise CommandError("The push to the base branch did not land on the expected commit")
...
def cleanup_worktree(repo_path: str, worktree_root: str, issue_number: int):
    ...
    if dirty.strip():
        return False,"Worktree has uncommitted changes; not removed."
    ...
    return True,f"Worktree removed. Branch kept: {branch}"
...
def ensure_baseline_worktree(repo_path: str, root: str, base_sha: str):
    ...
    if wt.exists():
        ...
        if p.returncode != 0:
            raise CommandError(f"Invalid baseline worktree: {wt}")
        current = p.stdout.strip()
        if current != base_sha:
            raise CommandError(f"Baseline worktree {wt} points to {current[:12]}, expected {base_sha[:12]}")
        dirty = status_porcelain(wt)
        if dirty.strip():
            raise CommandError(f"Baseline worktree is not clean: {wt}")
        return wt, False
...
def create_detached_worktree(repo_path: str | Path, worktree: str | Path, sha: str) -> Path:
    wt = Path(worktree)
    if wt.exists():
        raise CommandError(f"Temporary integration worktree already exists: {wt}")
...
def remove_detached_worktree_if_clean(repo_path: str | Path, worktree: str | Path) -> tuple[bool, str]:
    wt = Path(worktree)
    if not wt.exists():
        return True, "temporary worktree no longer exists"
    if cherry_pick_in_progress(wt):
        return False, "cherry-pick in progress; worktree preserved"
    dirty = status_porcelain(wt)
    if dirty.strip():
        return False, "temporary worktree has changes; preserved"
    git(repo_path, ["worktree", "remove", str(wt)])
...
def delete_issue_branch_if_merged(
    ...
    if wt.exists():
        dirty = status_porcelain(wt)
        if dirty.strip():
            messages.append(f"WARN worktree {wt} has uncommitted changes; preserved together with branch {branch}")
            return messages
        current = git(wt, ["branch", "--show-current"], check=False).stdout.strip()
        if current and current != branch:
            messages.append(f"WARN worktree {wt} is on unexpected branch {current}; preserved")
            return messages
        ...
        if rm.returncode == 0:
            ...
            messages.append(f"WARN could not remove worktree {wt} (a file may be locked): {detail}")

    if ref_exists(repo_path, f"refs/heads/{branch}"):
        local_sha = rev_parse(repo_path, branch)
        if local_sha != issue_commit:
            messages.append(f"WARN local branch {branch} moved to {local_sha[:12]} (expected {issue_commit[:12]}); not deleted")
        else:
            git(repo_path, ["branch", "-D", branch])
            messages.append(f"Local branch deleted: {branch}")
    ...
    if remote_p.returncode == 0 and remote_line:
        ...
        if remote_sha != issue_commit:
            messages.append(f"WARN remote branch origin/{branch} moved to {remote_sha[:12]} (expected {issue_commit[:12]}); not deleted")
        else:
            del_p = git(repo_path, ["push", "origin", "--delete", branch], check=False)
            if del_p.returncode == 0:
                messages.append(f"Remote branch deleted: origin/{branch}")
            else:
                messages.append(f"WARN could not delete origin/{branch}: {del_p.stdout}{del_p.stderr}")
    return messages
...
def push_head_to_base_if_unchanged(worktree: str | Path, base_branch: str, expected_remote_sha: str) -> str:
    ...
    if current != expected_remote_sha:
        raise CommandError(
            f"The remote base changed during validation: expected {expected_remote_sha[:12]}, "
            f"origin/{base_branch} is at {current[:12]}. Nothing was pushed."
        )
    head = rev_parse(worktree, "HEAD")
    git(worktree, ["push", "origin", f"HEAD:refs/heads/{base_branch}"])
    verify = remote_branch_sha(str(worktree), base_branch)
    if verify != head:
        raise CommandError(f"Push did not verify the expected SHA: HEAD={head[:12]} remote={verify[:12]}")
    return head
