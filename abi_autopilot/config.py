...
def default_config(
    ...
    return {
        ...
        # Prefijos de referencias nombradas en la sección "Dependencias" de una
        # issue, ej. ["UX", "API"] para `UX-12`. Se resuelven buscando
        # `[UX-12]` en el título. Vacío = sólo se reconocen `#N`.
        "dependency_ref_prefixes": [],
        "availability": {
            "enabled": True,
        ...
        "toolchain": {
            # Opcional: versiones que usa el CI del repositorio objetivo. Doctor
            # sólo avisa si difieren (salvo strict). Vacío = no se compara.
        ...
        "validation_workers": {
            # Vitest is safe to fan out. Pytest stays serial unless pytest-xdist
            # is installed in the target repo. Playwright stays serial because
            # E2E suites often share application state.
            "vitest": 4,
            "pytest": 1,
            "playwright": 1,
        },
        "integration": {
            ...
            "validate_fast": True,
            # Prefijos que cuentan como código. Vacío = todo lo que no sea
            # documentación (docs/, *.md) dispara validaciones.
            "code_paths": [],
        ...
        "release": {
            ...
            # temprana de semanas de divergencia silenciosa.
            "drift_warn_commits": 1,
            # Con True, la divergencia bloquea nuevos cierres como released.
            "block_close_on_drift": False,
            # Issues cerradas que revisa `release-audit` (auditoría de consistencia).
            "audit_limit": 1000,
        },
...
def load_config(path: Path) -> dict:
    if not path.exists():
        raise ConfigError(f"{path} does not exist. Run the `init` command first (see README, Step 3).")
...
def write_initial_config(
    ...
    """Write a new config. With a `detect.Detection`, its checks replace the
    example frontend/backend layout."""
    repo_path = Path(repo).expanduser().resolve()
    cfg = default_config(repo_path, repo_slug, base_branch, deployment_branch)
    if detection is not None:
        from .detect import apply_detection
        apply_detection(cfg, detection)
...
def upgrade_config(path: Path) -> dict:
    ...
    for spec in merged.get("workspace_bootstrap", []):
        if (
            ...
            spec.setdefault("cache_probe", spec.get("if_missing", "node_modules/.package-lock.json"))
    ...
    backup = path.with_suffix(path.suffix + f".pre-v{__version__}.bak")
