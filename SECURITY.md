# Seguridad — ABI Autopilot v1.8.1
...
- `integrate` es una acción explícita del operador y sólo acepta issues `agent:implemented` (o `agent:done` abiertas heredadas) con evidencia completa PASS.
- No force push.
...
- `.env`, `backend/data/` y lo que se agregue a `protected_paths` no pueden modificarse; los prompts de los agentes listan esas rutas.
...
No publiques credenciales, logs de `runtime/` ni detalles explotables en una issue pública. Escribí en privado a Gabriel Jaime, <gabrielsjaime@gmail.com>, y compartí únicamente la información mínima necesaria para reproducir el problema.

*English:* please do not open a public issue for vulnerabilities. Email Gabriel Jaime at <gabrielsjaime@gmail.com> with the minimum information needed to reproduce the problem, and never include credentials or `runtime/` logs.
